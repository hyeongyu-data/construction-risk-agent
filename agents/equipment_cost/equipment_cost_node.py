...
[정보 부족 처리 원칙]
- 장비 대기 비용 요청은 맞지만 필수 정보가 부족한 경우 status를 "MISSING_INFO"로 설정한다.
- 공종만 있고 장비명이 없는 경우에는 get_equipment_by_work_type을 호출해 조회한다.
- 대기일수가 전혀 없으면 계산하지 말고 missing_fields에 "대기일수"를 포함한다.
- 단, 사용자가 "개략", "대략", "예상", "추정"이라는 표현을 사용한 경우에는 기본값 3일로 계산하고, assumptions에 명시한다.
